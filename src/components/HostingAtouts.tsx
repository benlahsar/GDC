...
export const HostingAtouts: React.FC = () => {
  const t = useTranslations("HostingAtouts");
  const [isVisible, setIsVisible] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const sectionRef = useRef<HTMLElement>(null);

  const ATOUTS_DATA = [
    { id: 1, text: t("items.tracking"), icon: UserCheck },
    { id: 2, text: t("items.price"), icon: Tag },
    { id: 3, text: t("items.compliance"), icon: ShieldCheck },
    { id: 4, text: t("items.scalable"), icon: Layers },
    { id: 5, text: t("items.turnkey"), icon: Key },
    { id: 6, text: t("items.team"), icon: Users },
    { id: 7, text: t("items.satisfaction"), icon: Heart },
    { id: 8, text: t("items.tech"), icon: Cpu },
  ];
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-[1400px] relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
          {ATOUTS_DATA.map((item, index) => (
            <div
              key={item.id}
              className={`group relative flex flex-col items-center justify-center ${!isMobile ? 'transition-all duration-700 ease-out' : ''} ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}
            ...
            >
              ...
              {/* 3. CENTRAL CORE */}
              <div className={`relative z-10 w-32 h-32 rounded-full bg-[#F0F0F2] dark:bg-[#0A0A0A] border border-black/10 dark:border-white/10 flex items-center justify-center shadow-[inset_0_4px_20px_rgba(0,0,0,0.05)] dark:shadow-[inset_0_4px_20px_rgba(255,255,255,0.05)] transition-all duration-500 ease-out ${!isMobile ? 'group-hover:scale-110 group-hover:border-brand-red group-hover:shadow-[0_0_40px_rgba(220,38,38,0.2)]' : ''}`}>
                <item.icon
                  size={36}
