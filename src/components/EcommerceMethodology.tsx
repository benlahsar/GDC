...
export const EcommerceMethodology: React.FC = () => {
  const t = useTranslations("EcommerceMethodology");
  ...
  const STEPS = [
    {
      ...
      description: (t.raw("steps") as any[])[0].description,
      icon: Target
    },
    {
      id: "02",
      title: (t.raw("steps") as any[])[1].title,
      description: (t.raw("steps") as any[])[1].description,
      icon: Search
    },
    {
      id: "03",
      title: (t.raw("steps") as any[])[2].title,
      description: (t.raw("steps") as any[])[2].description,
      icon: Map
    },
    {
      id: "04",
      title: (t.raw("steps") as any[])[3].title,
      description: (t.raw("steps") as any[])[3].description,
      icon: Activity
    }
  ];
          ...
          <div className={`lg:col-span-5 ${!isMobile ? 'transition-all duration-1000' : ''} ${isVisible || isMobile ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="lg:sticky lg:top-32 flex flex-col justify-between h-full">
              <div>
                ...
                <h2
                  className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.9] mb-8 text-black dark:text-white"
              ...
              {STEPS.map((step, index) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative overflow-hidden rounded-[32px] border transition-all duration-500 cursor-pointer ${isActive ? 'bg-white dark:bg-[#0A0A0A] border-black/10 dark:border-brand-red/30 shadow-2xl dark:shadow-[0_0_40px_-10px_rgba(220,38,38,0.15)] scale-[1.01]' : 'bg-white/40 dark:bg-transparent border-black/5 dark:border-white/10 hover:bg-white/60 dark:hover:bg-white/5'}`}
                  >
                    ...
                    <div className="relative z-10 p-6 md:p-10">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center border transition-all duration-300 ${isActive ? 'bg-brand-red border-brand-red text-white rotate-180 shadow-lg' : 'border-black/10 dark:border-white/10 text-gray-400 dark:text-gray-500'}`}>
                          {isActive ? <Minus size={18} /> : <Plus size={18} />}
                        </div>
                      </div>

                      <div className={`grid transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isActive ? 'grid-rows-[1fr] opacity-100 mt-8' : 'grid-rows-[0fr] opacity-0 mt-0'}`}>
