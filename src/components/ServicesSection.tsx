...
export const ServicesSection: React.FC = () => {
  ...
  return (
    <section
      id="agency"
      className="relative w-full bg-white dark:bg-black transition-colors duration-500 overflow-hidden"
    >
        ...
        {!isMobile && (
          <>
            <div className="absolute inset-y-0 left-0 w-32 z-30 bg-gradient-to-r from-white via-white/80 dark:from-[#050505] dark:via-[#050505]/80 to-transparent flex items-center justify-start pl-8 pointer-events-none">
              <span className="text-brand-red font-mono text-sm font-bold tracking-widest opacity-80">
                [ + ]
              </span>
            </div>
          ...
          {[...Array(4)].map((_, setIndex) => (
            <div key={setIndex} className="flex items-center shrink-0">
              {SCROLL_ITEMS.map((item, idx) => (
                <div
                  key={`${setIndex}-${idx}`}
                  className="flex items-center px-8 md:px-20 relative"
                >
                  {/* Content */}
                  <span
          ...
          {SERVICES_DATA.map((service) => (
            ...
            >
              ...
              <div className="relative z-10 flex-grow flex flex-col gap-4 md:gap-5">
                ...
                {/* Description */}
                <p
                  className={`text-sm md:text-base leading-relaxed text-gray-500 dark:text-gray-400 transition-colors duration-300`}
                >
                  {service.description}
                </p>
              </div>
              ...
              >
                <div className="flex flex-col">
                  <span className="text-[9px] uppercase text-gray-400 font-semibold tracking-widest mb-1">
                    Impact
                  </span>
              ...
              {!isMobile && (
                <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-brand-red to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left"></div>
              )}
            </div>
          ))}
        </div>
