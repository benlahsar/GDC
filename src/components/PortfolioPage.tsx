...
import { 
  ...
  Search, Monitor, Smartphone, Palette, Globe, 
  BarChart3, Zap, ChevronRight, ShieldCheck, 
  TrendingUp, User, Mail, MessageSquare, Loader2,
  Maximize2, Eye, Layout, Code2, Box, Cpu, HardDrive,
  Check, Activity, ChevronLeft, ListChecks, DollarSign,
  Rocket, Award, Camera
} from 'lucide-react';
...
export const PortfolioPage: React.FC<PortfolioPageProps> = ({ onNavigate }) => {
  ...
  const handleBriefSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsSuccess(false);
      setFormStep(1);
      setFormData({ name: '', email: '', services: [], vision: '', budget: 'Standard' });
    }, 4000);
  };

  const filteredProjects = PROJECTS.filter(p => activeFilter === 'Tous' || p.filter === activeFilter);

  return (
    <div className="min-h-screen bg-[#F8F8FA] dark:bg-[#050505] pt-32 md:pt-48 lg:pt-56 pb-0 transition-colors duration-1000 relative overflow-x-hidden font-sans selection:bg-brand-red selection:text-white">
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 max-w-[1800px] relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-16 md:mb-24 lg:mb-40">
            <div className={`transition-all duration-1000 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className="inline-flex items-center gap-3 px-6 py-2.5 rounded-full bg-white dark:bg-white/5 backdrop-blur-2xl border border-black/5 dark:border-white/10 mb-12 shadow-xl">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.6em] text-gray-800 dark:text-gray-200">GDC SHOWCASE • ELITE</span>
                </div>
                ...
                <h1 className="text-[13vw] lg:text-[12rem] font-black text-black dark:text-white tracking-tighter leading-[0.8] uppercase mb-16 relative">
                    ARCHIVE <br/>
                    <span className="text-brand-red italic text-[11vw] lg:text-[10rem]">D'IMPACT.</span>
                </h1>
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-12 max-w-6xl mx-auto">
                    ...
                    <p className="text-lg md:text-2xl text-gray-600 dark:text-gray-400 font-medium max-w-2xl uppercase tracking-tighter">
                       Ingénierie Digitale <span className="text-black dark:text-white font-black">&</span> Esthétique Pure.
                    </p>
                    <div className="h-px flex-1 bg-black/10 dark:bg-white/10 hidden md:block"></div>
                </div>
            </div>
            ...
            <div className={`mt-16 md:mt-24 w-full flex justify-center ${isMobile ? '' : 'opacity-0 animate-enter-bottom delay-500'}`}>
                <div className="max-w-full overflow-x-auto hide-scrollbar p-1.5 md:p-2 rounded-[2.5rem] bg-white/70 dark:bg-black/40 backdrop-blur-3xl border border-black/5 dark:border-white/10 shadow-2xl flex items-center">
                    <div className="flex items-center gap-1 px-1">
                        {['Tous', 'Web', 'Image de Marque', 'SEO', 'Contenu'].map((f) => (
                            <button 
                                key={f}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 auto-rows-fr">
            ...
            {/* INTEGRATED STATS */}
            <div className={`lg:col-span-4 bg-[#0A0A0A] rounded-[48px] p-10 flex flex-col justify-between relative overflow-hidden group shadow-2xl border border-white/5 transition-all duration-700 ${isMobile ? '' : 'opacity-0 animate-enter-bottom'}`} style={{ animationDelay: '600ms' }}>
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500">Live Analytics</span>
                    </div>
                    <div className="space-y-6">
                        <AnimatedCounter value={200} prefix="+" className="text-7xl font-black text-white tracking-tighter leading-none" />
                        <p className="text-sm text-gray-400 font-bold uppercase tracking-widest">Solutions High-Perf</p>
                    </div>
                </div>
                <div className="relative z-10 pt-10 border-t border-white/5 flex justify-between">
                    <div className="flex flex-col"><span className="text-2xl font-black text-brand-red">0.8s</span><span className="text-[8px] font-black uppercase text-gray-500">Avg. Load</span></div>
                    <div className="flex flex-col"><span className="text-2xl font-black text-white">98%</span><span className="text-[8px] font-black uppercase text-gray-500">ROI Score</span></div>
                </div>
            </div>

            {/* 3-STEP BRIEF FORM */}
            <div className={`lg:col-span-8 bg-white dark:bg-[#0A0A0A] rounded-[48px] p-8 md:p-12 border border-black/5 dark:border-white/10 shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[550px] ${isMobile ? '' : 'opacity-0 animate-enter-bottom'}`} style={{ animationDelay: '800ms' }}>
                <div className="relative z-10 h-full flex flex-col">
                    <div className="flex items-center justify-between mb-12">
                        <div className="flex flex-col">
                            <h3 className="text-3xl font-black text-black dark:text-white uppercase tracking-tighter leading-none mb-2">Initialiser le Brief</h3>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Step {formStep} of 3</p>
                        </div>
                        <div className="flex gap-2">
                            {[1, 2, 3].map(s => (
                                <div key={s} className={`w-10 h-1.5 rounded-full transition-all duration-500 ${formStep >= s ? 'bg-brand-red' : 'bg-black/5 dark:bg-white/10'}`}></div>
                            ))}
                        </div>
                    </div>
                    ...
                    {isSuccess ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center animate-enter-zoom py-10">
                            ...
                            <p className="text-2xl font-black text-black dark:text-white uppercase tracking-tighter">Protocole Activé.</p>
                            <p className="text-sm text-gray-500 mt-2">Nous analysons vos données. Réponse sous 12h.</p>
                        </div>
                    ) : (
                        <form onSubmit={handleBriefSubmit} className="flex-1 flex flex-col gap-8">
                            {formStep === 1 && (
                                <div className="space-y-10 animate-enter-right">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-2 group">
                                            <label className="text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 group-focus-within:text-brand-red transition-colors">Identité</label>
                                            <input 
                                              type="text" required placeholder="John Doe / Brand"
                                        ...
                                        <div className="space-y-2 group">
                                            <label className="text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 group-focus-within:text-brand-red transition-colors">Contact Pro</label>
                                            <input 
                                              type="email" required placeholder="email@direct.com"
                                              className="w-full bg-transparent border-b border-black/10 dark:border-white/10 py-4 text-xl font-black outline-none focus:border-brand-red transition-all text-black dark:text-white"
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 animate-enter-right">
                                    {FORM_SERVICES.map(s => (
                                        <button 
                                          ...
                                          className={`p-6 rounded-[2rem] border transition-all duration-300 flex flex-col items-center gap-3 text-center ${formData.services.includes(s.label) ? 'bg-brand-red border-brand-red text-white shadow-xl' : 'bg-black/5 dark:bg-white/5 border-transparent text-gray-500 hover:text-black dark:hover:text-white'}`}
                                        >
                            ...
                            {formStep === 3 && (
                                <div className="space-y-8 animate-enter-right flex-1">
                                    <div className="space-y-2 group">
                                        <label className="text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 group-focus-within:text-brand-red transition-colors">Vision du Projet</label>
                                        <textarea 
                            ...
                            <div className="mt-auto pt-8 border-t border-black/5 dark:border-white/5 flex justify-between items-center">
                                {formStep > 1 ? (
                                    <button type="button" onClick={handleBack} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-black dark:hover:text-white">
                                ...
                                {formStep < 3 ? (
                                    <button 
                                      ...
                                      disabled={formStep === 1 && (!formData.name || !formData.email)}
                                    ...
                                    >
                                        {isSubmitting ? <Loader2 size={16} className="animate-spin" /> : <>Déployer <ArrowRight size={16} /></>}
                                    </button>
                                )}
                            </div>
                        </form>
        ...
        <section className={`mt-24 md:mt-64 relative rounded-[40px] md:rounded-[120px] p-10 md:p-32 bg-black dark:bg-white text-white dark:text-black overflow-hidden group shadow-[0_60px_150px_-30px_rgba(255,0,0,0.4)] transition-all duration-1000 ${isMobile ? 'opacity-100' : 'opacity-0 animate-enter-bottom'}`}>
            <div className="absolute inset-0 bg-noise opacity-[0.08] mix-blend-overlay"></div>
            
            {/* Immersive Mobile Glow */}
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] bg-brand-red rounded-full blur-[100px] md:blur-[300px] opacity-20 transition-all duration-[3s] group-hover:opacity-40 animate-pulse`}></div>
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
                <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full border border-white/20 dark:border-black/10 bg-white/5 dark:bg-black/5 backdrop-blur-md mb-10 md:mb-16">
                    ...
                    <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em]">START THE REVOLUTION</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-8 w-full max-w-sm">
                   ...
                   <a href="#contact" className="group/cta relative w-full sm:w-auto inline-flex items-center justify-center gap-8 md:gap-12 px-8 py-7 md:px-14 md:py-10 bg-brand-red text-white rounded-full font-black uppercase tracking-[0.4em] text-xs md:text-base hover:scale-110 active:scale-95 transition-all shadow-[0_20px_60px_-10px_rgba(255,0,0,0.5)] overflow-hidden">
                ...
                <div className="mt-16 md:mt-24 flex items-center justify-center gap-6 md:gap-8 opacity-20 grayscale transition-all duration-700 hover:opacity-100 hover:grayscale-0">
                    <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest">Digital Mastery</span>
                    <div className="w-1.5 h-1.5 rounded-full bg-brand-red"></div>
                    <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest">Global Impact</span>
                    <div className="w-1.5 h-1.5 rounded-full bg-brand-red"></div>
                    <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest">ROI Focused</span>
                </div>
            </div>
        </section>
...
const ProjectBentoCard: React.FC<{ project: any; index: number; isMobile: boolean; onClick: () => void; }> = ({ project, index, isMobile, onClick }) => {
    ...
    return (
        <div 
            ref={cardRef}
            className={`${project.size} group/card relative rounded-[40px] overflow-hidden 
            ...
            style={{ 
                ...
                transform: !isMobile && isHovered ? `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg) scale3d(1.02, 1.02, 1)` : 'none'
            }}
            ...
            onClick={onClick}
        >
            <div className={`absolute inset-0 z-0 transition-all duration-1000 ${isMobile ? 'opacity-100' : 'opacity-0 group-hover/card:opacity-100'}`}>
                <div className={`absolute inset-0 bg-gradient-to-br ${project.accent}`}></div>
            ...
            <div className="relative z-10 p-6 md:p-8 flex justify-between items-start">
                 <div className="flex flex-col gap-1">
                    ...
                    <span className="text-lg font-mono font-black opacity-5 group-hover/card:text-brand-red group-hover/card:opacity-100 transition-all">{project.id}</span>
                 </div>
            ...
            <div className="relative z-10 flex-1 flex items-center justify-center p-8">
                 <img src={project.logo} alt={project.title} className="max-w-[70%] max-h-[140px] object-contain transition-all duration-700 grayscale group-hover:grayscale-0 group-hover:scale-110" />
            </div>
            ...
            <div className="relative z-10 p-8 pt-0 transition-all duration-500 transform translate-y-4 group-hover:card:translate-y-0 opacity-0 group-hover:card:opacity-100">
                <h3 className="text-xl font-black text-black dark:text-white uppercase tracking-tighter mb-2">{project.title}</h3>
                <div className="flex flex-wrap gap-2">
                    {project.services.map((s: string, i: number) => (
                        <span key={i} className="px-3 py-1 bg-black/5 dark:bg-white/5 text-[8px] font-black uppercase tracking-widest">{s}</span>
                    ))}
                </div>
            </div>
        </div>
