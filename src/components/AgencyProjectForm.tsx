...
import { 
  ...
  Building2, User, Mail, Users, ArrowLeft,
  Layout, Search, Zap, Share2, Palette, Globe,
  MessageSquare, DollarSign, Send, Loader2
} from 'lucide-react';
...
const SERVICES = [
  { id: 'web', label: 'Site Web', icon: Layout },
...
export const AgencyProjectForm: React.FC<{ isMobile?: boolean }> = ({ isMobile }) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <div className="space-y-8 animate-enter-left">
            ...
            <div className="pt-8 flex items-center gap-4">
               ...
               <div>
                  <p className="font-bold text-black dark:text-white">Rejoignez nos +50 clients satisfaits</p>
                  <div className="flex gap-1 text-brand-red text-xs">
                     ★★★★★
                  </div>
               </div>
            </div>
            ...
            <div className="relative bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 rounded-[32px] p-8 md:p-10 shadow-2xl overflow-hidden transition-colors duration-500">
              ...
              <div className="flex items-center justify-between mb-10 relative">
                 ...
                 {[
                   ...
                   return (
                     <div key={s.id} className="relative z-10 flex flex-col items-center gap-2 bg-white dark:bg-[#0A0A0A] px-2 transition-colors duration-500">
                        <div className={`
                           w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300
                           ${isActive || isCompleted ? 'bg-brand-red text-white scale-110 shadow-[0_0_15px_rgba(220,38,38,0.5)]' : 'bg-gray-100 dark:bg-white/10 text-gray-400 dark:text-gray-500'}
                        `}>
                           {isCompleted ? <Check size={14} /> : s.id}
                        </div>
                        <span className={`text-[10px] font-bold uppercase tracking-widest ${isActive ? 'text-black dark:text-white' : 'text-gray-400 dark:text-gray-600'}`}>
                           {s.label}
                        </span>
                     </div>
                   )
              ...
              {isSuccess ? (
                 <div className="py-20 flex flex-col items-center text-center animate-enter-zoom">
                    ...
                    <h3 className="text-2xl font-bold text-black dark:text-white mb-2">Demande Envoyée !</h3>
                    <p className="text-gray-600 dark:text-gray-400 mb-8">Nous reviendrons vers vous sous 24h.</p>
                    <button 
                      onClick={() => {
                          setIsSuccess(false); 
                          setStep(1);
                  ...
                  <div className="min-h-[380px]">
                      ...
                      <div className="space-y-5 animate-fade-in-up">
                        <div className="group space-y-2">
                           <div className="relative">
                              <input 
                                ...
                                required
                              />
                              <User className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-600" size={18} />
                           </div>
                        </div>
                        ...
                        <div className="group space-y-2">
                           <div className="relative">
                              <input 
                                ...
                                required
                              />
                              <Mail className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-600" size={18} />
                           </div>
                        </div>
                        ...
                        <div className="group space-y-2">
                           <div className="relative">
                              <input 
                                ...
                                className="w-full bg-transparent border border-black/10 dark:border-white/20 rounded-xl px-5 py-4 text-black dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:border-brand-red focus:outline-none focus:bg-black/5 dark:focus:bg-white/5 transition-all"
                              />
                              <Building2 className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-600" size={18} />
                           </div>
                        </div>
                        ...
                        <div className="group space-y-2">
                           ...
                           <div className="relative">
                              ...
                              >
                                {EMPLOYEES.map(opt => <option key={opt} value={opt} className="bg-white dark:bg-black text-black dark:text-white">{opt}</option>)}
                              </select>
                        ...
                        <div className="group space-y-2">
                           <div className="flex relative" ref={countryRef}>
                              ...
                              >
                                <span className="text-sm font-bold text-black dark:text-white">{selectedCountry.code}</span>
                    ...
                    {step === 2 && (
                      <div className="animate-fade-in-up">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-6 block">Service souhaité</label>
                           ...
                           {SERVICES.map((srv) => {
                             ...
                             return (
                               <div 
                                 ...
                                 `}
                               >
                                 <srv.icon size={24} className={isSelected ? 'text-white' : 'text-gray-500 dark:text-gray-400 group-hover:text-black dark:group-hover:text-white transition-colors'} />
                                 <span className={`text-xs font-bold uppercase tracking-wider ${isSelected ? 'text-white' : 'text-gray-600 dark:text-gray-300'}`}>{srv.label}</span>
                               </div>
                             )
                           })}
                        </div>
                      ...
                      <div className="space-y-6 animate-fade-in-up">
                         <div className="space-y-2">
                            ...
                            <textarea 
                              ...
                              placeholder="Racontez-nous vos objectifs..."
                            ></textarea>
                         </div>
