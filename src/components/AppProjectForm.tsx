...
export const AppProjectForm: React.FC = () => {
  const t = useTranslations("AppProjectForm");
  const COUNTRY_CODES = t.raw("country_codes") as any[];
  const EMPLOYEES = t.raw("employees_ranges") as string[];
  const SERVICES = t.raw("services_list") as string[];
  const BUDGETS = t.raw("budgets_ranges") as string[];
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 max-w-[1700px] relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center">
          ...
          <div className="space-y-10">
            <h2
              className="text-5xl md:text-7xl lg:text-[5.5rem] font-black tracking-tighter leading-[0.95] text-black dark:text-white uppercase transition-colors duration-500"
              dangerouslySetInnerHTML={{
                __html: t.raw("brading.title").replace(/{br}/g, "<br/>"),
              }}
            />
            ...
            <div className="flex items-center gap-4 pt-6 border-t border-black/10 dark:border-white/10">
              <div className="flex -space-x-4">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="w-12 h-12 rounded-full border-2 border-white dark:border-black bg-gray-200 dark:bg-gray-800 overflow-hidden"
                  >
                    <img
          ...
          <div className="relative">
            ...
            <div className="relative bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 rounded-[40px] shadow-2xl overflow-hidden transition-all duration-500">
              ...
              <div className="flex items-center justify-between p-8 md:p-12 pb-0 relative">
                ...
                {(t.raw("stepper") as any[]).map((s) => {
                  const isActive = step >= s.id;
                  return (
                    <div
                      key={s.id}
                      className="relative z-10 flex flex-col items-center gap-3 bg-white dark:bg-[#0A0A0A] px-4 transition-colors duration-500"
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-500 border-2 ${
                          isActive
                            ...
                            : "bg-transparent border-black/10 dark:border-white/10 text-gray-400 dark:text-gray-600"
                        }`}
                      >
                        {step > s.id ? (
                      ...
                      >
                        {s.label.split(" ")[0]} <br className="md:hidden" />{" "}
                        {s.label.split(" ")[1] || ""}
                      </span>
                    </div>
                  );
                ...
                {isSuccess ? (
                  <div className="py-20 flex flex-col items-center text-center animate-enter-zoom">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-xs mx-auto">
                      Nos experts analysent votre demande et reviennent vers
                      vous sous 24h.
                    </p>
                    <button
                      onClick={() => {
                      ...
                      className="px-8 py-3 bg-brand-red text-white rounded-xl font-bold uppercase tracking-widest text-xs hover:scale-105 transition-transform shadow-lg"
                    >
                      Nouveau projet
                    </button>
                  </div>
                        ...
                        <div className="space-y-5 animate-fade-in-up">
                          <div className="relative group">
                            ...
                            <input
                              ...
                              className="w-full bg-gray-50 dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-xl py-4 pl-12 pr-4 text-black dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:border-brand-red focus:outline-none transition-all"
                            />
                          </div>
                          <div className="relative group">
                            <Mail
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="relative">
                              <select
                                ...
                                className="w-full bg-gray-50 dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-xl py-4 px-4 text-black dark:text-white focus:border-brand-red focus:outline-none appearance-none cursor-pointer"
                              >
                                {EMPLOYEES.map((e) => (
                                  <option
                                    key={e}
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all duration-300 ${
                                    isSelected
                                      ? "bg-brand-red/10 border-brand-red/30 text-brand-red dark:text-white"
                                      : "bg-black/5 dark:bg-white/5 border-transparent text-gray-600 dark:text-gray-400 hover:bg-black/10 dark:hover:bg-white/10"
                                  }`}
                                >
                                  <div
                      ...
                      {step === 3 && (
                        <div className="space-y-5 animate-fade-in-up">
                          <input
                            ...
                            className="w-full bg-gray-50 dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-xl py-4 px-5 text-black dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:border-brand-red focus:outline-none transition-all"
                          />
                          <textarea
                            ...
                            className="w-full bg-gray-50 dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-xl py-4 px-5 text-black dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:border-brand-red focus:outline-none transition-all resize-none"
                          />
                    ...
                    <div className="flex items-center justify-between pt-8 mt-4 border-t border-black/5 dark:border-white/10">
                      {step > 1 ? (
                        <button
                          type="button"
                          onClick={handleBack}
                          className="flex items-center gap-2 px-8 py-3 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-black dark:text-white rounded-xl font-bold uppercase tracking-widest text-[10px] transition-all"
                        >
