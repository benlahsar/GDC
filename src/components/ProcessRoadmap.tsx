...
const STEPS = [
  {
    ...
    desc: "Étude approfondie de votre secteur à Marrakech, analyse concurrentielle et définition d'une stratégie digitale alignée avec vos objectifs."
  },
  {
    id: 2,
    icon: PenTool,
    title: "Design & UX",
    desc: "Création d'interfaces modernes et intuitives qui reflètent votre identité visuelle. Conception optimisée pour tous les appareils."
  },
  {
    id: 3,
    icon: Check,
    title: "Développement & Optimisation",
    desc: "Développement technique performant, structure HTML sémantique et intégration SEO avancée pour une performance maximale."
  }
];
    ...
    <div className="relative bg-white dark:bg-[#020202] transition-colors duration-500">
        ...
        <div className="md:hidden py-16 px-4">
             <div className="text-center mb-12">
                <h2 className="text-3xl font-black text-black dark:text-white tracking-tight mb-4 leading-tight">
                    Processus de <br/><span className="text-brand-red">Création</span>
                </h2>
                <p className="text-sm text-gray-600 dark:text-gray-300 font-medium leading-relaxed">
                   3 étapes clés pour un site web performant à Marrakech.
                </p>
             </div>
             ...
             <div className="flex flex-col gap-8 relative">
                 ...
                 {STEPS.map((step, index) => (
                     <div key={index} className="relative z-10 flex flex-col items-center text-center bg-gray-50 dark:bg-[#080808] p-6 rounded-2xl border border-black/5 dark:border-white/5 shadow-sm">
        ...
        {/* === DESKTOP LAYOUT === */}
        <section ref={containerRef} className="hidden md:block relative h-[175vh]">
            <div ref={stickyRef} className="sticky top-0 h-screen overflow-hidden flex flex-col justify-center items-center bg-white dark:bg-[#020202] transition-colors duration-500">
                ...
                >
                    <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-black dark:text-white tracking-tight mb-4 leading-tight">
                        Processus de <span className="text-brand-red">Création Site Web Marrakech</span>
                    </h2>
                    <div className="w-24 h-1.5 bg-brand-red rounded-full mb-8"></div>
                    <p className="max-w-4xl mx-auto text-base md:text-xl text-gray-600 dark:text-gray-400 font-medium leading-relaxed">
                        Notre méthodologie éprouvée garantit des résultats concrets pour votre projet <br className="hidden md:block"/>
                        de <span className="text-brand-red font-bold">création site web Marrakech</span>
                    </p>
                </div>
                ...
                <div className="relative w-full max-w-[1200px] px-4 md:px-8 flex items-center justify-center z-10 mt-20 md:mt-0">
                    <div className="absolute top-1/2 left-0 w-full -translate-y-1/2 flex items-center pointer-events-none z-0 px-10 md:px-20">
                         ...
                         <div 
                            ...
                            style={{ 
                                ...
                                background: 'linear-gradient(90deg, #FF0000 0%, #FF0000 66%, #10B981 100%)',
                    ...
                    <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12 relative z-10">
                        {STEPS.map((step, index) => {
                            ...
                            if (index === 1 && progress > 0.40) isActive = true;
                            ...
                            const inactiveClass = "border-black/5 dark:border-white/5 bg-gray-50 dark:bg-[#050505] opacity-50 grayscale";

                            return (
                                <div 
                                    key={index}
                                    id={`step-card-${step.id}`}
                                    className={`relative group p-8 md:p-10 rounded-[24px] border flex flex-col items-center text-center transition-all duration-700 ${isActive ? `${activeGlowClass} scale-100 translate-y-0` : `${inactiveClass} scale-95 translate-y-8`}`}
                                >
                                    <div className={`w-20 h-20 rounded-full border-2 flex items-center justify-center mb-6 relative transition-all duration-500 ${isActive ? (isGreen ? 'border-emerald-500 text-emerald-500 bg-emerald-500/10' : 'border-brand-red text-brand-red bg-brand-red/10') : 'border-black/10 dark:border-white/10 text-gray-400 dark:text-gray-500 bg-transparent'}`}>
                                        <step.icon size={32} strokeWidth={1.5} />
                                        {isActive && <div className={`absolute inset-0 rounded-full animate-ping opacity-20 ${isGreen ? 'bg-emerald-500' : 'bg-brand-red'}`}></div>}
                                    </div>
                                    <h3 className={`text-xl font-bold mb-4 uppercase tracking-wide ${isActive ? 'text-black dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>{step.title}</h3>
                                    <div className={`w-12 h-[1px] mb-6 transition-colors duration-500 ${isActive ? (isGreen ? 'bg-emerald-500' : 'bg-brand-red') : 'bg-black/10 dark:bg-white/10'}`}></div>
                                    <p className={`text-sm leading-relaxed font-medium ${isActive ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400 dark:text-gray-600'}`}>{step.desc}</p>
                                </div>
                            )
                        })}
                    </div>
                ...
                <div className="absolute inset-0 pointer-events-none z-50 overflow-hidden">
                    {particles.map(p => (
                        <div key={p.id} style={{ position: 'absolute', left: p.x, top: p.y, width: p.size, height: p.size, backgroundColor: p.color, borderRadius: '50%', opacity: p.life, transform: `translate(-50%, -50%) scale(${p.life})`, boxShadow: `0 0 ${p.size * 2}px ${p.color}` }} />
                    ))}
                </div>
            </div>
        </section>
        ...
        <section className="relative pb-24 pt-0 px-4 bg-white dark:bg-[#020202] transition-colors duration-500 -mt-0 md:-mt-52 z-20">
            <div className="max-w-5xl mx-auto rounded-[50px] p-12 md:p-16 text-center overflow-hidden border border-black/5 dark:border-white/10 bg-gray-50 dark:bg-gradient-to-br dark:from-[#111] dark:via-[#050505] dark:to-black relative group shadow-xl dark:shadow-2xl">
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black mb-8 leading-tight text-black dark:text-white">
                        Prêt à lancer votre projet de <br/>
                        <span className="text-brand-red">création site web Marrakech</span> ?
                    </h2>
                    <p className="max-w-3xl mx-auto text-lg md:text-xl text-gray-600 dark:text-gray-400 mb-12 font-light">
                        Notre agence experte en création site web Marrakech vous accompagne de la conception à la mise en ligne, avec une stratégie SEO intégrée pour maximiser votre visibilité.
                    </p>
                    <a href="#contact" className="inline-flex items-center gap-4 px-10 py-5 bg-brand-red text-white rounded-full font-black uppercase tracking-widest text-sm hover:scale-105 hover:bg-red-600 hover:shadow-[0_0_40px_rgba(255,0,0,0.4)] transition-all duration-300 group">
                        Démarrer mon projet
