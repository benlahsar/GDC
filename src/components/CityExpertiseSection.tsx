  ...
  return (
    <section className="relative w-full py-24 md:py-32 bg-[#F0F0F2] dark:bg-[#000000] overflow-hidden transition-colors duration-500 border-t border-black/5 dark:border-white/5">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          className={`absolute top-1/4 right-0 w-[800px] h-[800px] bg-brand-red/[0.02] rounded-full blur-[120px] pointer-events-none`}
        ></div>
        <div
          className={`absolute bottom-1/4 left-0 w-[600px] h-[600px] bg-blue-500/[0.02] rounded-full blur-[120px] pointer-events-none`}
        ></div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 max-w-[1600px] relative z-10">
        ...
        <div className="text-center mb-16 md:mb-24">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-black/5 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-md mb-6 shadow-sm">
            <MapPin
              size={14}
              className={`text-brand-red ${!isMobile ? "animate-bounce" : ""}`}
            />
          ...
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: t
                .raw("description")
                .replace("<highlight>", '<span class="text-brand-red">')
          ...
          {EXPERTISE_DATA.map((item) => (
            <div
              ...
              className={`${item.colSpan} ${
                item.height
              } group relative rounded-[40px] overflow-hidden border border-black/5 dark:border-white/10 bg-white dark:bg-[#050505] transition-all duration-700 ease-out ${
                !isMobile
                  ? "hover:shadow-[0_20px_60px_-10px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_20px_60px_-10px_rgba(255,255,255,0.05)] hover:-translate-y-1"
                  : ""
            ...
            >
              ...
              <div className="relative overflow-hidden z-0 w-full md:w-1/2 h-[250px] md:h-full">
                ...
                {!isMobile && (
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-transparent to-white dark:to-[#050505] hidden md:block w-full h-full"></div>
                )}
              ...
              <div className="relative z-20 p-8 md:p-12 flex flex-col justify-between w-full md:w-1/2 bg-white dark:bg-[#050505]">
                ...
                <div>
                  ...
                  <p className="text-brand-red font-bold text-xs uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Zap
                      size={12}
                      className={`${!isMobile ? "fill-brand-red" : ""}`}
                    />{" "}
                    {item.subtitle}
                  </p>

                  <h3
                    className={`font-black text-black dark:text-white mb-4 leading-tight text-2xl md:text-3xl lg:text-4xl ${
                ...
                <div className="flex flex-wrap gap-2 mt-auto pt-8 border-t border-black/5 dark:border-white/5">
                  {(item.features as string[]).map((feature, idx) => (
                    <div
                      key={idx}
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-white/5 border border-black/5 dark:border-white/5 text-[11px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide ${
                        !isMobile ? "group-hover:border-brand-red/30" : ""
              ...
              {/* Hover Border */}
              {!isMobile && (
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-brand-red/20 rounded-[40px] pointer-events-none transition-colors duration-500 z-30"></div>
              )}
            </div>
          ))}
        </div>
