                            ...
                            <div className="p-6 rounded-[24px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-xl mt-8 group relative overflow-hidden">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-lg text-black dark:text-white">{t("support.title")}</h4>
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-4">
                        {FAQS.map((faq, index) => (
                            <div
                                ...
                                className={`group relative rounded-[24px] transition-all duration-500 cursor-pointer ${openIndex === index ? 'bg-white dark:bg-[#0A0A0A] shadow-2xl scale-[1.01] z-10 ring-1 ring-black/5 dark:ring-white/10' : 'bg-transparent hover:bg-white/40 dark:hover:bg-white/5'}`}
                            >
                                <div className="p-6 md:p-8">
                                    <div className="flex items-start justify-between gap-6">
                                        ...
                                        <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-500 ${openIndex === index ? 'bg-brand-red border-brand-red text-white rotate-180' : 'border-black/10 dark:border-white/10 text-gray-400 dark:text-gray-500 group-hover:border-black dark:group-hover:border-white group-hover:text-black dark:group-hover:text-white'}`}>
                                            {openIndex === index ? <Minus size={14} /> : <Plus size={14} />}
                                        </div>
                                    </div>

                                    <div
                                        className={`grid transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${openIndex === index ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0 mt-0'}`}
                                    >
