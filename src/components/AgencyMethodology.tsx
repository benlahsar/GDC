...
export const AgencyMethodology: React.FC<{ isMobile?: boolean }> = ({
  isMobile,
}) => {
  const t = useTranslations("AgencyMethodology");
  // Need to reconstruct the steps array with translations
  const STEPS = STEPS_CONFIG.map((config) => {
    const stepT = t.raw(`steps.${config.id}`) as any;
    return {
      ...config,
      title: stepT.title,
      subtitle: stepT.subtitle,
      description: stepT.description,
      details: stepT.details as string[],
    };
  });
  ...
  return (
    <section className="relative w-full py-24 md:py-40 bg-[#F8F8FA] dark:bg-[#020202] transition-colors duration-700 overflow-hidden border-y border-black/5 dark:border-white/5">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] bg-brand-red/[0.02] rounded-full blur-[150px] transition-opacity duration-1000 ${
            hoveredStep === "01" || hoveredStep === "04"
              ? "opacity-100"
              : "opacity-40"
          }`}
        ></div>
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 max-w-[1700px] relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-24 md:mb-32">
          ...
          <h2 className="text-5xl md:text-7xl lg:text-9xl font-black tracking-tighter leading-[0.85] text-black dark:text-white uppercase italic flex flex-wrap justify-center">
            {t("title.prefix")}
            <span className="inline-block w-4 md:w-8 lg:w-12">&nbsp;</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {STEPS.map((step, idx) => (
            ...
            >
              {/* Background Decor */}
              <div
                className={`absolute -top-24 -right-24 w-64 h-64 bg-gradient-to-br ${step.color} opacity-0 group-hover:opacity-10 blur-3xl transition-opacity duration-700`}
              ></div>
              ...
              <div className="relative z-10 mt-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {step.details.map((detail, dIdx) => (
                    <div
                      key={dIdx}
                      className="px-4 py-1.5 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 flex items-center gap-2"
                    >
              ...
              <div className="relative z-10 mt-12 pt-8 border-t border-black/5 dark:border-white/10 flex items-center justify-between opacity-40 group-hover:opacity-100 transition-opacity duration-500">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest">
                    {t("cta.active_process")}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-black uppercase tracking-[0.2em]">
                    {t("cta.module")} {step.id}
                  </span>
                  {!isMobile && (
                    <ArrowRight
                      size={14}
        ...
        <div className="mt-20 flex flex-col md:flex-row items-center justify-between gap-8 p-10 md:p-14 rounded-[48px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="text-center md:text-left">
              <h3 className="text-3xl md:text-4xl font-black text-black dark:text-white tracking-tighter uppercase leading-none mb-2">
                {t("cta.title")}
              </h3>
