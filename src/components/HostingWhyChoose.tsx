...
export const HostingWhyChoose: React.FC = () => {
  const t = useTranslations("HostingWhyChoose");
  ...
  const REASONS = [
    {
      id: "performance",
      title: t("reasons.performance.title"),
      desc: t("reasons.performance.desc"),
      icon: Zap,
    ...
    {
      id: "hosting",
      title: t("reasons.hosting.title"),
      desc: t("reasons.hosting.desc"),
      icon: Server,
      color: "text-blue-500",
      bg: "bg-blue-500/10",
    },
    {
      id: "backup",
      title: t("reasons.backup.title"),
      desc: t("reasons.backup.desc"),
      icon: Database,
    ...
    {
      id: "engagement",
      title: t("reasons.engagement.title"),
      desc: t("reasons.engagement.desc"),
      icon: HeartHandshake,
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full py-24 md:py-32 bg-[#F0F0F2] dark:bg-[#050505] text-black dark:text-white overflow-hidden border-t border-black/5 dark:border-white/5 transition-colors duration-500"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div
            className={`lg:col-span-7 flex flex-col gap-4 ${
              !isMobile ? "transition-all duration-1000 delay-200" : ""
            } ${
              isVisible
          ...
          >
            {REASONS.map((item, index) => (
              <div
                key={item.id}
                className={`group relative p-6 md:p-8 rounded-[24px] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/5 transition-all duration-300 overflow-hidden ${
                  !isMobile
                    ? "hover:border-black/10 dark:hover:border-white/20 hover:shadow-lg hover:bg-white dark:hover:bg-black"
                    : ""
                }`}
              >
                <div className="relative z-10 flex gap-5 items-start">
                  <div
                    className={`shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${
                      item.bg
                    } bg-opacity-10 border border-black/5 dark:border-white/10 ${
                      item.color
                    } transition-all duration-300 ${
                      !isMobile
                        ? "group-hover:scale-110 group-hover:bg-white dark:group-hover:bg-black group-hover:border-transparent group-hover:shadow-md"
                        : ""
                    }`}
                  >
                  ...
                  <div className="flex-1">
                    <h3
                      className={`text-xl font-bold mb-2 text-black dark:text-white transition-colors flex items-center gap-2`}
                    >
                      {item.title}
                      {!isMobile && (
                        <ChevronRight
                          size={16}
                          className={`opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 ${item.color}`}
                        />
                      )}
                    </h3>
          ...
          <div
            className={`lg:col-span-5 flex flex-col gap-6 ${
              !isMobile ? "transition-all duration-1000 delay-400" : ""
            } ${
              isVisible
                ? "opacity-100 translate-x-0"
                : "opacity-0 translate-x-10"
          ...
          >
            {/* 1. VISION CARD */}
            <div className="relative p-8 md:p-10 rounded-[40px] bg-[#111] dark:bg-[#111] text-white border border-white/10 overflow-hidden group shadow-2xl">
                ...
                <div className="space-y-4 text-gray-300 text-sm md:text-base leading-relaxed font-medium">
                  <p
                    dangerouslySetInnerHTML={{
                      __html: t
                        .raw("vision.p1")
                        .replace(/<highlight>/g, `<strong class="text-white">`)
            ...
            {/* 2. MISSION CARD */}
            <div className="relative p-8 md:p-10 rounded-[40px] bg-white text-black border border-black/5 overflow-hidden group shadow-xl">
              <div className="relative z-10">
