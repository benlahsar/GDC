...
import { 
  Target, Eye, Zap, ShieldCheck, HeartHandshake, Rocket, 
  CheckCircle2, ShoppingBag, CreditCard, BarChart3, Globe 
} from 'lucide-react';
            ...
            ">
                <div className="absolute inset-0 z-0">
                    <img 
                        ...
                        className={`w-full h-full object-cover opacity-80 dark:opacity-60 ${!isMobile ? 'transition-transform duration-[2s] ease-out group-hover:scale-105' : ''}`}
                    />
                ...
                <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 w-full max-w-5xl px-8">
                    {[
                        { icon: ShoppingBag, label: "Boutique", color: "text-blue-500" },
                        { icon: CreditCard, label: "Paiements", color: "text-emerald-500" },
                        ...
                        <div key={i} className="flex flex-col items-center gap-4 group/icon">
                            <div className={`w-20 h-20 md:w-24 md:h-24 rounded-[30px] bg-white/80 dark:bg-black/60 backdrop-blur-xl border border-white/40 dark:border-white/10 flex items-center justify-center shadow-2xl transition-transform duration-500 ${!isMobile ? 'group-hover/icon:-translate-y-4 group-hover/icon:scale-110' : ''}`}>
                                <item.icon size={40} className={`${item.color} drop-shadow-lg`} strokeWidth={1.5} />
                            </div>
            ...
            ">
                <div className="relative z-10">
                    <p className="text-base md:text-lg font-medium text-gray-600 dark:text-gray-300 mb-10 leading-relaxed">
                        Chez <strong className="text-black dark:text-white">Group Digital Concept</strong>, nous sommes des partenaires stratégiques dédiés à la réussite de votre projet e-commerce.
                    </p>

                    <div className="mb-10">
                        <h3 className="text-2xl font-black text-black dark:text-white mb-6 flex items-center gap-3">
                            <Zap className="text-brand-red fill-brand-red" /> Ce Que Nous Vous Offrons
                        </h3>
                        <div className="space-y-6">
                            {[
                                { title: "Expertise Technique & Marketing", desc: "Équipe pluridisciplinaire maîtrisant Shopify, WooCommerce et PrestaShop." },
                                { title: "Approche 100% Sur Mesure", desc: "Analyse de votre secteur et de vos concurrents." },
                                { title: "Performance & Sécurité", desc: "Sites ultra-rapides et protégés contre les cybermenaces." },
                                { title: "Support Continu", desc: "Suivi réactif et maintenance proactive." }
                            ].map((item, i) => (
                                <div key={i} className="flex gap-4 group">
                                    <div className={`mt-1 w-6 h-6 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center shrink-0 transition-colors duration-300 ${!isMobile ? 'group-hover:bg-brand-red group-hover:text-white' : ''}`}>
                                        <CheckCircle2 size={14} />
                                    </div>
                                    <div>
                                        <h4 className="text-base font-bold text-black dark:text-white mb-1">{item.title}</h4>
            ...
            <div className="col-span-1 lg:col-span-5 flex flex-col gap-6">
                <div className="flex-1 rounded-[40px] p-8 md:p-10 bg-gradient-to-br from-black to-[#111] dark:from-[#111] dark:to-black text-white border border-white/10 relative overflow-hidden group shadow-xl">
                    ...
                    <div className="relative z-10 h-full flex flex-col">
                        ...
                        <h3 className="text-3xl font-serif italic mb-4 text-white">Vision</h3>
                        <p className="text-white/80 leading-relaxed font-medium text-lg">
                            Devenir le <span className="text-brand-red font-bold">partenaire de référence</span> en e-commerce à Marrakech. 
                        </p>
                    </div>
                </div>
                ...
                <div className="flex-1 rounded-[40px] p-8 md:p-10 bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 relative overflow-hidden group shadow-xl">
                    <div className="relative z-10 h-full flex flex-col">
                        ...
                        <h3 className="text-3xl font-serif italic mb-6 text-black dark:text-white">Mission</h3>
                        <div className="space-y-4">
                            <div className="flex gap-4">
                                <div className="w-1 h-full min-h-[40px] bg-brand-red rounded-full"></div>
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-300">
                                    <strong className="text-black dark:text-white block mb-1">Maximiser la visibilité</strong>
