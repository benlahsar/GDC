...
const getProjects = (t: any) => [
  {
    ...
    category: t("projects.mmreseaux.category"),
    ...
    desc: t("projects.mmreseaux.desc"),
    tags: ["WordPress", "UI/UX", "SEO Local"],
    year: "2024",
  },
  {
    id: 2,
    title: "Group Plancy Call",
    category: t("projects.plancycall.category"),
    ...
    desc: t("projects.plancycall.desc"),
    tags: ["Branding", "Lead Gen", "Performance"],
    year: "2024",
  },
  {
    id: 3,
    title: "Compagnons Bâtiment",
    category: t("projects.compagnons.category"),
    ...
    desc: t("projects.compagnons.desc"),
    tags: ["Portfolio", "Architecture", "SEO"],
    year: "2023",
  },
  {
    id: 4,
    title: "Majorelle Centre",
    category: t("projects.majorelle.category"),
    ...
    desc: t("projects.majorelle.desc"),
    tags: ["Réservation", "Corporate", "Marrakech"],
    year: "2023",
  },
];
...
export const ProjectsSection: React.FC = () => {
  ...
  return (
    <section
      ...
      className="relative w-full py-32 bg-[#F0F0F2] dark:bg-[#000000] overflow-hidden border-t border-black/5 dark:border-white/5"
    >
      <div className="absolute inset-0 z-0 pointer-events-none">
        {!isMobile && (
          <div
            ...
            style={{
              backgroundImage: `radial-gradient(600px circle at ${cursorPos.x}px ${cursorPos.y}px, rgba(255, 0, 0, 0.15), transparent 40%), linear-gradient(rgba(0,0,0,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(0,0,0,0.1) 1px, transparent 1px)`,
      ...
      {activeProject && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
          ...
          <div className="relative w-full max-w-6xl h-full md:h-auto md:max-h-[90vh] bg-[#F0F0F2] dark:bg-[#0A0A0A] rounded-[40px] overflow-hidden shadow-2xl border border-black/5 dark:border-white/10 flex flex-col md:flex-row animate-enter-bottom">
            <button
              onClick={() => setActiveProject(null)}
              className="absolute top-6 right-6 z-50 w-12 h-12 rounded-full bg-black/10 dark:bg-white/10 flex items-center justify-center hover:bg-brand-red hover:text-white transition-colors"
            >
              <X size={24} />
            </button>
            ...
            <div className="w-full md:w-1/2 p-10 md:p-16 flex flex-col justify-center">
              ...
              <a
                ...
                className="group w-full py-5 bg-brand-red text-white rounded-xl font-bold uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-red-600 transition-all shadow-lg"
              >
                {t("cta")} <ExternalLink size={18} />
              </a>
            </div>
          </div>
        </div>
...
const ProjectCard: React.FC<{
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    setRotation({
      x: (mouseY / rect.height - 0.5) * -25,
      y: (mouseX / rect.width - 0.5) * 25,
    });
  };

  return (
    <div
      className="group/card relative h-[450px] w-full perspective-2000 cursor-pointer"
    ...
    >
      <div
        ref={cardRef}
        className={`relative w-full h-full rounded-[40px] bg-white/5 dark:bg-white/5 backdrop-blur-xl border border-white/20 dark:border-white/10 shadow-xl transition-all duration-300 ease-out overflow-hidden`}
        style={{
          transform:
            !isMobile && isHovered
              ? `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg) scale3d(1.05, 1.05, 1)`
              : "none",
      ...
      >
        ...
        <div
          className={`absolute inset-0 rounded-[40px] border-2 border-transparent ${
            !isMobile ? "group-hover/card:border-brand-red/30" : ""
        ...
        >
          <img
            ...
            className={`w-full h-full object-contain drop-shadow-2xl transition-all duration-500 ${
              !isMobile ? "group-hover/card:scale-110" : ""
            }`}
          />
        </div>
        <div
          className={`absolute bottom-6 right-6 ${
            !isMobile ? "opacity-0 group-hover/card:opacity-100" : "opacity-100"
