            ...
            <div>
              ...
              <h2
                ...
                dangerouslySetInnerHTML={{
                  __html: t
                    .raw("title")
                    .replace("{br}", '<br class="hidden lg:block"/>')
                    .replace(
                      "{span}",
                      `<span class="text-transparent bg-clip-text bg-gradient-to-r from-brand-red to-red-600">${t.raw(
                        "titleHighlight"
                      )}</span>`
                    ),
                }}
              ...
              {(optimisations as any[]).map((item, index) => (
                <div key={index} className="flex gap-4 md:gap-5 items-start">
                  <div className="mt-1.5 flex-shrink-0">
                    <Play
                      size={14}
                      className="text-brand-red fill-brand-red rotate-0 group-hover:rotate-90 transition-transform duration-300"
                    />
                  </div>
                  <div>
              ...
              <ul className="space-y-5 relative z-10">
                {(seoItems as string[]).map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <Star
                      size={16}
                      className="text-brand-red fill-brand-red shrink-0 mt-1 animate-pulse-slow"
                    />
                    <span className="font-medium text-sm md:text-base text-gray-300 group-hover:text-white transition-colors">
                      {item}
                    </span>
                  </li>
                ))}
              ...
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: t
                    ...
                    .replace(
                      "{span}",
                      `<span class="text-brand-red font-bold not-italic">${
                        locale === "fr"
                          ? "création site web Marrakech"
                          : locale === "es"
                          ? "creación sitio web Marrakech"
                          : "website creation Marrakech"
                      }</span>`
                    ),
                }}
              />
