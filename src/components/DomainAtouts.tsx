...
export const DomainAtouts: React.FC = () => {
  const t = useTranslations("DomainAtouts");
  const [isVisible, setIsVisible] = useState(false);
  const sectionRef = useRef<HTMLElement>(null);

  const ATOUTS_DATA = [
    { id: 1, text: t("items.0"), icon: Users },
    { id: 2, text: t("items.1"), icon: Key },
    { id: 3, text: t("items.2"), icon: Layers },
    { id: 4, text: t("items.3"), icon: Tag },
    { id: 5, text: t("items.4"), icon: Cpu },
    { id: 6, text: t("items.5"), icon: ShieldCheck },
    { id: 7, text: t("items.6"), icon: Heart },
    { id: 8, text: t("items.7"), icon: UserCheck },
  ];
    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-[1400px] relative z-10">
        ...
        >
          ...
          <h2
            className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter text-black dark:text-white mb-6 leading-tight max-w-5xl mx-auto"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
          {ATOUTS_DATA.map((item, index) => (
            <div
              key={item.id}
              className={`relative group flex flex-col items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                isVisible ? "opacity-100 scale-100" : "opacity-0 scale-50"
              }`}
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              {/* THE MORPHING BUBBLE */}
              <div className="relative w-40 h-40 md:w-48 md:h-48 lg:w-56 lg:h-56 flex items-center justify-center transition-all duration-500 ease-out">
                {/* 1. Base Circle (White/Glass) - Morphs to Rounded Square on Hover */}
                <div className="absolute inset-0 bg-white text-black rounded-full group-hover:rounded-[40px] shadow-[0_10px_30px_rgba(0,0,0,0.05)] dark:shadow-[0_0_40px_rgba(255,255,255,0.1)] group-hover:shadow-[0_20px_60px_rgba(220,38,38,0.4)] transition-all duration-500 ease-out group-hover:bg-gradient-to-br group-hover:from-brand-red group-hover:to-[#990000] group-hover:scale-110 border-4 border-white dark:border-white/5"></div>
                ...
                <div className="relative z-10 flex flex-col items-center text-center px-4 transition-transform duration-500">
                  ...
                  <div className="h-0 overflow-hidden opacity-0 group-hover:h-auto group-hover:opacity-100 group-hover:mb-3 transition-all duration-500 delay-75">
                    <item.icon
                      size={32}
                  ...
                  {/* Text */}
                  <span className="text-sm md:text-base lg:text-lg font-bold uppercase tracking-tight leading-tight text-black group-hover:text-white transition-colors duration-300">
                    {item.text}
                  </span>
              ...
              <div className="w-20 h-4 rounded-[100%] bg-black/10 dark:bg-black/50 blur-lg mt-4 transition-all duration-500 group-hover:w-32 group-hover:bg-brand-red/30 group-hover:blur-xl group-hover:translate-y-2"></div>
            </div>
          ))}
        </div>
        ...
        >
          ...
          <div className="flex justify-center gap-2 text-[10px] font-bold uppercase tracking-widest text-brand-red">
            <span>• {t("footer.tags.0")}</span>
            <span>• {t("footer.tags.1")}</span>
            <span>• {t("footer.tags.2")}</span>
          </div>
        </div>
      </div>
    </section>
