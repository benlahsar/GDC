      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 max-w-[1700px] relative z-10">
        ...
        <div className={`text-center mb-16 md:mb-24 ${!isMobile ? 'opacity-0 animate-enter-bottom' : 'opacity-100'}`}>
           <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-black/5 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-md mb-6 shadow-sm">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-gray-800 dark:text-gray-200">Portfolio Client</span>
           </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
            {PROJECTS_DATA.map((project, idx) => (
                <div 
                  key={project.id}
                  className={`group relative flex flex-col items-center ${!isMobile ? 'opacity-0 animate-enter-bottom' : 'opacity-100'}`}
                ...
                >
                    ...
                    <div className={`
                        ...
                        ${!isMobile ? 'hover:-translate-y-4 hover:border-brand-red/30 shadow-xl hover:shadow-2xl' : 'shadow-lg'}
                    `}
                    style={{ isolation: 'isolate', transform: 'translateZ(0)' }}
                    >
                        {/* Dynamic Background Glow */}
                        <div className={`absolute inset-0 bg-gradient-to-br ${project.accent} opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
                        ...
                        {/* Project Logo */}
                        <div className={`relative z-10 w-full h-full flex items-center justify-center ${!isMobile ? 'animate-float' : ''}`} style={!isMobile ? { animationDelay: `${idx * 500}ms` } : {}}>
                            <img 
                                src={project.logo} 
                                alt={project.title} 
                                className={`w-full h-full object-contain filter drop-shadow-2xl transition-transform duration-500 ${!isMobile ? 'group-hover:scale-110' : ''}`}
                            />
                        </div>

                        {/* Floating Action Button */}
                        <div className={`absolute bottom-6 right-6 w-12 h-12 rounded-full bg-brand-red text-white flex items-center justify-center transition-all duration-500 delay-100 shadow-lg shadow-red-600/40 z-20 ${!isMobile ? 'opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0' : 'opacity-100'}`}>
                    ...
                    <div className="mt-8 text-center px-4">
                        <h3 className={`text-base md:text-lg font-black text-black dark:text-white uppercase tracking-tight leading-tight transition-colors ${!isMobile ? 'group-hover:text-brand-red' : ''}`}>
                            {project.title}
                        </h3>
                        {/* Subtle decorative dot */}
                        <div className={`w-1.5 h-1.5 rounded-full bg-brand-red/20 mx-auto mt-3 transition-all ${!isMobile ? 'group-hover:bg-brand-red group-hover:scale-150' : 'bg-brand-red'}`}></div>
                    </div>
                </div>
            ))}
        </div>
