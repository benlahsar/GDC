        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {STATS.map((stat, idx) => (
            ...
            >
              {/* Hover Glow */}
              <div
                className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-500 ${stat.bg.replace(
                  "10",
                  "20"
                )}`}
              ></div>
              ...
              {/* Icon */}
              <div
                className={`w-16 h-16 rounded-2xl ${stat.bg} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500`}
              >
              ...
              <div className="flex items-baseline gap-1 mb-2">
                <AnimatedCounter
                  value={Number(t(`stats.items.${stat.key}.val`))}
                  prefix={t(`stats.items.${stat.key}.prefix`)}
