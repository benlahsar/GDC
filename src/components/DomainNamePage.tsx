...
export const DomainNamePage: React.FC = () => {
  ...
  const handleCheckDomain = async (
    ...
    const fullDomain = targetDomain.includes(".")
      ? targetDomain
      : `${targetDomain}.ma`;
    ...
    try {
      ...
      if (finalAvailable) {
        const mxResponse = await fetch(
          `https://dns.google/resolve?name=${fullDomain}&type=MX`
        );
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full min-h-screen bg-[#F0F0F2] dark:bg-[#050505] text-black dark:text-white pt-48 md:pt-48 pb-0 overflow-hidden transition-colors duration-500 selection:bg-brand-red selection:text-white"
    >
      {/* --- RESERVATION MODAL --- */}
      {isReserving && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-8">
          <div
            className="absolute inset-0 bg-black/80 backdrop-blur-xl animate-enter-zoom"
          ...
          <div className="relative w-full max-w-xl bg-white dark:bg-[#0E0E0E] rounded-[40px] border border-black/10 dark:border-white/10 shadow-2xl overflow-hidden p-8 md:p-12 animate-enter-bottom">
            <button
              onClick={() => setIsReserving(false)}
              className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center hover:bg-brand-red hover:text-white transition-all"
            >
              <X size={20} />
            </button>
            ...
            {resSuccess ? (
              <div className="text-center py-10 animate-fade-in-up">
                ...
                <p className="text-gray-500 dark:text-gray-400">
                  {t
                    .raw("modal.success.desc")
                    .replace("{domain}", searchResult?.domain || "")}
                </p>
                <p className="text-xs mt-6 text-gray-400 font-bold uppercase tracking-widest">
              ...
              <div className="space-y-8">
                  ...
                  <h3
                    ...
                    dangerouslySetInnerHTML={{
                      __html: t
                        ...
                        .replace(
                          "{domain}",
                          `<span class='text-brand-red italic'>${searchResult?.domain}</span>`
                        )
                        .replace(/{br}/g, "<br/>"),
                    }}
                  />
                ...
                <form onSubmit={handleReserveSubmit} className="space-y-4">
                  <div className="space-y-2 group">
                    <label className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 ml-2 group-focus-within:text-brand-red transition-colors">
                      {t("modal.fields.nameLabel")}
                    </label>
                    <div className="relative">
                      <User
                        className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-black dark:group-focus-within:text-white"
                        size={18}
                      />
                      <input
                        type="text"
                  ...
                  >
                    {isResSubmitting ? (
                      ...
                      <>
                        {t("modal.fields.button")} <Send size={16} />
                      </>
                    )}
                  </button>
                </form>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className={`absolute top-[-10%] left-[-10%] w-[80vw] h-[80vw] max-w-[800px] max-h-[800px] bg-brand-red/[0.04] rounded-full blur-[120px] ${
            !isMobile
        ...
        ></div>
        <div
          className={`absolute bottom-0 right-0 w-[70vw] h-[70vw] max-w-[600px] max-h-[600px] bg-blue-600/[0.03] rounded-full blur-[150px] ${
            !isMobile
              ? "transition-transform duration-[2000ms] ease-out will-change-transform"
              : "opacity-40"
          }`}
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-[1600px] relative z-10 pb-20">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="relative mb-12 max-w-xl group/searchbox">
                  ...
                  >
                    ...
                    >
                      {isChecking ? (
                        ...
                        <>
                          {t("search.button")} <ArrowRight size={14} />
                        </>
                      )}
                    </button>
                  </div>
                ...
                {searchResult && (
                  <div
                    className={`mt-4 p-5 rounded-3xl animate-fade-in-up flex items-center justify-between border shadow-2xl backdrop-blur-xl ${
          ...
          >
            ...
            <div className="relative z-10 mt-10">
              <div
                ...
                className={`bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[2rem] p-8 flex items-center gap-5 shadow-2xl transition-all duration-500 cursor-text ${
                  !isMobile
                    ? "group-hover:scale-105 group-hover:border-white/20"
                    : ""
                } ${isChecking ? "ring-2 ring-brand-red" : ""}`}
              >
                {isChecking ? (
                  <Loader2 className="text-brand-red animate-spin" size={28} />
                ) : (
                ...
                <span
                  className={`text-xl font-black uppercase tracking-tighter transition-colors ${
                    domainInput ? "text-white" : "text-white/20"
                  }`}
                >
            ...
            <div className="relative z-10 mt-auto">
              <h3
                ...
                dangerouslySetInnerHTML={{
                  __html: t.raw("visual.title").replace(/{br}/g, "<br/>"),
                }}
              />
        ...
        <div
          className={`grid grid-cols-1 lg:grid-cols-12 gap-6 ${
            !isMobile ? "opacity-0 animate-fade-in-up delay-400" : ""
          }`}
        >
          {/* STATS */}
          ...
          <div className="col-span-1 lg:col-span-5 bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 rounded-[40px] p-8 flex flex-wrap content-center justify-center gap-4">
            {[
              ...
              >
                <div
                  className={`w-14 h-14 rounded-2xl bg-gray-50 dark:bg-[#151515] border border-black/5 dark:border-white/5 flex items-center justify-center text-gray-400 transition-all duration-300 shadow-sm ${
                    !isMobile
                      ? "group-hover:bg-brand-red group-hover:text-white group-hover:shadow-lg group-hover:-translate-y-1"
                      : ""
                  }`}
                >
                  <item.icon size={24} />
                ...
                <span
                  className={`text-[10px] font-bold uppercase tracking-wide text-gray-400 transition-colors ${
                    !isMobile
