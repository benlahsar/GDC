...
export const MaintenanceFormSection: React.FC = () => {
  const t = useTranslations("MaintenanceFormSection");
  ...
  return (
    <section className="relative w-full py-24 md:py-32 bg-[#F4F4F5] dark:bg-[#000000] text-black dark:text-white overflow-hidden transition-colors duration-500 border-t border-black/5 dark:border-white/5">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className={`absolute top-0 right-0 w-[800px] h-[800px] bg-brand-red/[0.03] rounded-full blur-[150px] -translate-y-1/2 translate-x-1/3 ${isMobile ? "opacity-10" : ""}`}
        ></div>
        <div
          className={`absolute bottom-0 left-0 w-[600px] h-[600px] bg-blue-600/[0.02] rounded-full blur-[120px] translate-y-1/2 -translate-x-1/4 ${isMobile ? "opacity-10" : ""}`}
        ></div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-[1600px] relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          {/* LEFT: CONTENT */}
          <div
            className={`lg:col-span-5 space-y-8 ${!isMobile ? "animate-enter-left" : ""}`}
          >
            <div className="relative">
              <h2
                className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-[1.1] text-black dark:text-white"
                dangerouslySetInnerHTML={{
                  __html:
                    t.raw("title").replace(/{br}/g, "<br/>") +
                    `<span class="inline-flex ml-2 w-3 h-3 bg-brand-red rounded-full ${!isMobile ? "animate-pulse shadow-[0_0_15px_rgba(255,0,0,0.8)]" : ""}"></span>`,
                }}
              />
            </div>
            ...
            <div className="pt-8 border-t border-black/5 dark:border-white/10 flex items-center gap-6">
              <div className="flex -space-x-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="w-12 h-12 rounded-full border-2 border-white dark:border-[#000] bg-gray-200 dark:bg-[#111] overflow-hidden"
                  >
                    <img
          ...
          {/* RIGHT: FORM CARD */}
          <div
            className={`lg:col-span-7 relative ${!isMobile ? "animate-enter-right delay-200" : ""}`}
          >
            {!isMobile && (
            ...
            <div
              className={`relative bg-white dark:bg-[#080808] border border-black/5 dark:border-white/10 rounded-[40px] p-8 md:p-12 shadow-2xl overflow-hidden transition-all duration-500`}
            >
              {/* Stepper */}
              {!isSuccess && (
                <div className="flex items-center justify-between mb-12 relative px-4">
                  ...
                  {[
                    ...
                    { id: 2, label: t("steps.1.label") },
                    { id: 3, label: t("steps.2.label") },
                  ].map((s) => {
                    ...
                    return (
                      <div
                        key={s.id}
                        className="relative z-10 flex flex-col items-center gap-3 bg-white dark:bg-[#080808] px-2 transition-colors"
                      >
                        <div
                          className={`
                        ...
                        <span
                          className={`text-[10px] font-bold uppercase tracking-widest text-center ${isActive ? "text-black dark:text-white" : "text-gray-400 dark:text-gray-600"}`}
                        >
                          {s.label}
                        </span>
                      </div>
              ...
              {isSuccess ? (
                <div className="py-20 text-center animate-enter-zoom">
                  ...
                  <button
                    ...
                    className="px-10 py-4 bg-brand-red text-white rounded-xl font-bold uppercase tracking-widest text-xs hover:scale-105 transition-transform shadow-lg"
                  >
                    {t("success.cta")}
                  </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="min-h-[380px] relative overflow-hidden">
                    {/* STEP 1 */}
                    <div
                      className={`transition-all duration-500 ease-in-out flex flex-col gap-5 ${step === 1 ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-full absolute inset-0 pointer-events-none"}`}
                    >
                    ...
                    {/* STEP 2 */}
                    <div
                      className={`absolute inset-0 transition-all duration-500 ease-in-out flex flex-col gap-4 ${step === 2 ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full pointer-events-none"}`}
                    >
                      <div className="grid grid-cols-2 gap-4">
                        {(t.raw("services") as any[]).map((s, idx) => {
                            ...
                            >
                              <SIcon
                                ...
                                className={
                                  isSelected
                                    ? "text-white"
                                    : `${!isMobile ? "group-hover:text-black dark:group-hover:text-white" : ""} transition-colors`
                                }
                              />
                              <span
                                className={`text-[10px] font-bold uppercase tracking-wider text-center ${isSelected ? "text-white" : ""}`}
                              >
                                {s.label}
                              </span>
                            </div>
                    ...
                    {/* STEP 3 */}
                    <div
                      className={`absolute inset-0 transition-all duration-500 ease-in-out flex flex-col ${step === 3 ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full pointer-events-none"}`}
                    >
                      <div className="space-y-4">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                          <MessageSquare size={14} /> {t("fields.detailsLabel")}
                        </label>
                        <textarea
