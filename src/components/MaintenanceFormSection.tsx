          ...
          <div className={`lg:col-span-5 space-y-8 ${!isMobile ? 'animate-enter-left' : ''}`}>
            <div className="relative">
              <h2
                ...
                dangerouslySetInnerHTML={{ __html: t.raw("title").replace(/{br}/g, "<br/>") + `<span class="inline-flex ml-2 w-3 h-3 bg-brand-red rounded-full ${!isMobile ? 'animate-pulse shadow-[0_0_15px_rgba(255,0,0,0.8)]' : ''}"></span>` }}
              />
            </div>
            ...
            <div className="pt-8 border-t border-black/5 dark:border-white/10 flex items-center gap-6">
              ...
              <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">{t("experts_count")}</p>
            </div>
          </div>
            ...
            <div className={`relative bg-white dark:bg-[#080808] border border-black/5 dark:border-white/10 rounded-[40px] p-8 md:p-12 shadow-2xl overflow-hidden transition-all duration-500`}>
              ...
              {isSuccess ? (
                <div className="py-20 text-center animate-enter-zoom">
                  ...
                  <h3 className="text-3xl font-black mb-2 text-black dark:text-white">{t("success.title")}</h3>
                  <p className="text-gray-500 dark:text-gray-400 mb-8">{t("success.desc")}</p>
                  <button onClick={() => { setIsSuccess(false); setStep(1); }} className="px-10 py-4 bg-brand-red text-white rounded-xl font-bold uppercase tracking-widest text-xs hover:scale-105 transition-transform shadow-lg">{t("success.cta")}</button>
                </div>
              ) : (
                  ...
                  <div className="min-h-[380px] relative overflow-hidden">
                    ...
                    <div className={`transition-all duration-500 ease-in-out flex flex-col gap-5 ${step === 1 ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full absolute inset-0 pointer-events-none'}`}>
                      <div className="space-y-4">
                        <input
                          type="text" placeholder={t("fields.name")} required
                        ...
                        />
                        <input
                          type="email" placeholder={t("fields.email")} required
                          value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })}
                          className="w-full bg-gray-50 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-5 py-4 text-black dark:text-white focus:border-brand-red focus:outline-none transition-all shadow-inner"
                        />
                        <input
                          type="text" placeholder={t("fields.company")}
                          value={formData.company} onChange={e => setFormData({ ...formData, company: e.target.value })}
                          className="w-full bg-gray-50 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-5 py-4 text-black dark:text-white focus:border-brand-red focus:outline-none transition-all shadow-inner"
                        />
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">{t("fields.employees")} <span className="text-brand-red">*</span></label>
                          <div className="relative">
                            <select
                        ...
                        <div className="flex relative" ref={countryRef}>
                          ...
                          <input
                            type="tel" placeholder={t("fields.phone")} required
                    ...
                    <div className={`absolute inset-0 transition-all duration-500 ease-in-out flex flex-col gap-4 ${step === 2 ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full pointer-events-none'}`}>
                      <div className="grid grid-cols-2 gap-4">
                        {(t.raw("services") as any[]).map((s, idx) => {
                          ...
                          const SIcon = [Layout, Search, ShieldCheck, Server, Zap, Globe][idx % 6];
                          return (
                            <div
                            ...
                            >
                              <SIcon size={24} className={isSelected ? 'text-white' : `${!isMobile ? 'group-hover:text-black dark:group-hover:text-white' : ''} transition-colors`} />
