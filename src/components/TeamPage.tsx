...
import { 
  Crown, Code2, Palette, Sparkles, Facebook, Linkedin, Github, 
  Plus, ArrowRight, Zap, Target, ShieldCheck
} from 'lucide-react';
...
const TEAM_MEMBERS: TeamMember[] = [
  {
    name: "Drome Ludovic",
    role: "CEO & Founder",
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766590955322-scaled.webp",
    category: 'executive',
    level: 0
  },
  {
    ...
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766582125139-scaled.webp",
    category: 'dev',
    level: 1
  },
  {
    ...
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766587028427-scaled.webp",
    category: 'design',
    level: 1
  },
  {
    ...
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766581123204-scaled.webp",
    category: 'dev',
    level: 2
  },
  {
    ...
    role: "Développeur Full-Stack",
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766580829355-scaled.webp",
    category: 'dev',
    level: 2
  },
  {
    ...
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766585274717-scaled.webp",
    category: 'dev',
    level: 2
  },
  {
    ...
    role: "UI/UX Designer",
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766585583307-scaled.webp",
    category: 'design',
    level: 2
  },
  {
    ...
    image: "https://groupdigitalconcept.com/wp-content/uploads/2025/12/creation-1766586638539-scaled.webp",
    category: 'design',
    level: 2
  }
];

const MemberCard: React.FC<{ member: TeamMember; delay?: number; isMobile: boolean }> = ({ member, delay = 0, isMobile }) => {
  ...
  const Icon = member.category === 'executive' ? Crown : member.category === 'dev' ? Code2 : Palette;
  const accentColor = member.category === 'executive' ? 'brand-red' : member.category === 'dev' ? 'blue-500' : 'purple-500';
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        transform: !isMobile && isHovered ? `rotateX(${rotate.x}deg) rotateY(${rotate.y}deg)` : 'none',
    ...
    >
      ...
      {!isMobile && (
        <div className={`absolute inset-0 bg-${accentColor}/20 blur-[60px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none -z-10`}></div>
      )}
      ...
      >
        ...
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute top-4 left-4 md:top-6 md:left-6 z-30">
            <div className={`p-3 md:p-4 rounded-[2rem] backdrop-blur-2xl bg-black/40 dark:bg-white/5 border border-white/20 text-white shadow-2xl transition-all duration-500 md:group-hover:bg-${accentColor} md:group-hover:scale-110 md:group-hover:rotate-12`}>
          ...
          <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 p-4 md:p-6 rounded-[2.5rem] bg-white/10 dark:bg-black/40 backdrop-blur-3xl border border-white/20 shadow-2xl z-30 flex flex-col items-center text-center transition-all duration-500 md:group-hover:border-white/40">
             <div className="mb-2">
                <p className={`text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] text-${accentColor} mb-1 flex items-center justify-center gap-2`}>
                    {isResponsable ? (
                      <span className="text-brand-red border border-brand-red/30 px-2 py-0.5 rounded-md bg-brand-red/5">RESPONSABLE</span>
                    ) : (
                      member.category
                    )}
...
export const TeamPage: React.FC = () => {
  ...
  const devLead = TEAM_MEMBERS.find(m => m.level === 1 && m.category === 'dev');
  const designLead = TEAM_MEMBERS.find(m => m.level === 1 && m.category === 'design');
  
  const devTeam = TEAM_MEMBERS.filter(m => m.level === 2 && m.category === 'dev');
  const designTeam = TEAM_MEMBERS.filter(m => m.level === 2 && m.category === 'design');

  return (
    <div className="min-h-screen bg-[#F0F0F2] dark:bg-[#020202] transition-colors duration-1000 font-sans selection:bg-brand-red selection:text-white pb-20 md:pb-64 overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-12 relative z-10">
        ...
        <section className="pt-32 md:pt-64 pb-16 md:pb-40 text-center relative">
          <div className="relative z-10 flex flex-col items-center">
            <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full border border-black/5 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-2xl mb-8 md:mb-10 shadow-xl animate-fade-in-up">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.6em] text-gray-800 dark:text-gray-200">GDC HIERARCHICAL CORE</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-6xl md:text-[9rem] lg:text-[13rem] font-black text-black dark:text-white tracking-tighter leading-[0.9] md:leading-[0.8] uppercase mb-8 md:mb-12 animate-fade-in-up">
              L'Architecture <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-red via-red-500 to-red-800 italic drop-shadow-2xl">Humaine.</span>
            </h1>
            
            <p className="text-lg md:text-3xl text-gray-500 dark:text-gray-400 font-light max-w-4xl mx-auto leading-relaxed animate-fade-in-up" style={{ animationDelay: '200ms' }}>
              Une synergie d'expertises orchestrée pour la <strong className="text-black dark:text-white font-black">Suprématie Digitale</strong>.
            </p>
          </div>
        </section>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="relative mb-20 md:mb-40 group">
            ...
            <div className={`absolute top-full left-1/2 -translate-x-1/2 w-[2px] md:w-[3px] h-12 md:h-32 bg-gradient-to-b from-brand-red via-brand-red/40 to-transparent shadow-[0_0_20px_rgba(255,0,0,0.4)]`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 md:gap-32 lg:gap-8">
                ...
                <div className="flex flex-col items-center relative">
                    ...
                    <div className="relative pt-6 mb-20 md:mb-32">
                         {devLead && <MemberCard member={devLead} delay={300} isMobile={isMobile} />}
                         {/* Trunk to Team (Blue Glow for Dev) */}
                         <div className={`absolute top-full left-1/2 -translate-x-1/2 w-[2px] h-12 md:h-32 bg-gradient-to-b from-blue-500/60 via-blue-500/20 to-transparent`}>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-8 pt-12">
                            {devTeam.map((m, i) => (
                                <div key={m.name} className="relative flex flex-col items-center">
                                    ...
                                    <MemberCard member={m} delay={isMobile ? 0 : 500 + (i * 150)} isMobile={isMobile} />
                                </div>
                            ))}
                         </div>
                    </div>
                ...
                <div className="flex flex-col items-center relative mt-20 lg:mt-0">
                    ...
                    <div className="relative pt-6 mb-20 md:mb-32">
                         {designLead && <MemberCard member={designLead} delay={300} isMobile={isMobile} />}
                         {/* Trunk to Team (Purple Glow for Design) */}
                         <div className={`absolute top-full left-1/2 -translate-x-1/2 w-[2px] h-12 md:h-32 bg-gradient-to-b from-purple-500/60 via-purple-500/20 to-transparent`}>
        ...
        <section className="mt-24 md:mt-64 relative py-12 md:py-20 border-y border-black/5 dark:border-white/5 overflow-hidden group">
            <div className={`flex items-center whitespace-nowrap w-max ${isMobile ? 'animate-[scroll_40s_linear_infinite]' : 'animate-scroll'}`}>
                {[...Array(6)].map((_, i) => (
                    <span key={i} className="text-2xl md:text-7xl font-black uppercase tracking-tighter mx-10 md:mx-20 opacity-20 transition-opacity md:group-hover:opacity-100">
                        The Future is <span className="text-brand-red">Human</span> • Code with <span className="text-brand-red">Passion</span> • Design for <span className="text-brand-red">Impact</span>
                    </span>
                ))}
            </div>
        </section>
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-24">
            <div className="text-center lg:text-left max-w-3xl">
              <div className="flex items-center gap-4 mb-6 md:mb-10 justify-center lg:justify-start">
                  ...
                  <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.5em] text-gray-400">TALENT HUB</span>
              </div>
              <h2 className="text-4xl md:text-8xl lg:text-9xl font-black uppercase tracking-tighter mb-8 md:mb-10 leading-[0.9] italic">
                Sculptez le <br/> <span className="text-brand-red not-italic underline decoration-white/10 underline-offset-[12px] md:underline-offset-[20px]">Web.</span>
              </h2>
              <p className="text-lg md:text-3xl font-light opacity-80 leading-relaxed">
                Nous cherchons les architectes digitaux capables de redéfinir les standards de l'industrie. Votre place est ici.
              </p>
              
              <div className="mt-10 md:mt-16 grid grid-cols-2 gap-6 md:gap-8 max-w-md mx-auto lg:mx-0">
                  <div className="flex flex-col gap-2">
                      <span className="text-3xl md:text-4xl font-black tracking-tighter text-brand-red">04</span>
                      <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-gray-500">Postes Ouverts</span>
                  </div>
                  <div className="flex flex-col gap-2">
                      <span className="text-3xl md:text-4xl font-black tracking-tighter text-brand-red">24h</span>
                      <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-gray-500">Délai Review</span>
                  </div>
              </div>
            </div>
            ...
            >
               ...
               <ArrowRight size={32} className="relative z-10 transition-transform duration-500 group-hover/btn:translate-x-6 md:w-[40px] md:h-[40px]" />
