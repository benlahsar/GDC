...
const TESTIMONIALS = [
  {
    ...
    company: "Atlas Group",
    content:
      "Une transformation radicale de notre présence digitale. Group Digital Concept a su capturer l'essence de notre marque et la propulser au niveau supérieur. Le ROI a dépassé toutes nos attentes.",
    metrics: "+210% de Leads",
    stars: 5,
    tag: "Stratégie 360",
  },
  {
    ...
    content:
      "L'équipe a fait preuve d'une créativité exceptionnelle. Notre nouveau site e-commerce est non seulement magnifique, mais il convertit incroyablement bien. Un partenaire de confiance.",
    metrics: "x3 Taux de Conv.",
    stars: 5,
    tag: "E-Commerce",
  },
  {
    ...
    company: "TechMaroc",
    content:
      "Nous cherchions une agence capable de comprendre les défis d'une startup tech. Leur réactivité et leur expertise technique nous ont permis de lancer notre produit en un temps record.",
  ...
  },
  {
    ...
    content:
      "Les campagnes publicitaires gérées par l'agence ont un ciblage chirurgical. Nous avons considérablement réduit notre coût d'acquisition tout en augmentant nos volumes de vente.",
    metrics: "-40% CPA",
    stars: 5,
    tag: "Ads & Social",
  },
  {
    ...
    content:
      "Le rebranding complet de notre agence immobilière a été un franc succès. Nos clients nous perçoivent désormais comme le leader incontesté du marché du luxe à Marrakech.",
    metrics: "+150% Notoriété",
    stars: 5,
    tag: "Branding",
  },
  {
    ...
    content:
      "Une application web robuste et intuitive. L'équipe technique de GDC maîtrise parfaitement les enjeux de performance et de sécurité. Une collaboration fluide et efficace.",
    metrics: "100% Uptime",
    stars: 5,
    tag: "App Web",
  },
  {
    ...
    company: "Moda Style",
    content:
      "Grâce à leur stratégie SEO, nous sommes passés de la 5ème page à la 1ère position sur nos mots-clés stratégiques. Le trafic organique a explosé en moins de 6 mois.",
    metrics: "Top 1 Google",
    stars: 5,
    tag: "SEO",
  },
  {
    ...
    company: "Auto Elite",
    content:
      "Leur approche data-driven fait toute la différence. Chaque dirham investi est suivi et optimisé. C'est la première fois que nous avons une telle clarté sur nos performances digitales.",
    metrics: "ROAS x8",
    stars: 5,
    tag: "Performance",
  },
  {
    ...
    name: "Nadia Fassi",
    role: "Gérante",
    company: "Riad Excellence",
    content:
      "Notre taux de réservation directe a doublé grâce au nouveau site web et à la stratégie de contenu. Ils ont su raconter notre histoire avec élégance et modernité.",
...
const ReviewCard: React.FC<{ review: (typeof TESTIMONIALS)[0] }> = ({
  review,
}) => (
  <div className="relative group p-6 md:p-8 rounded-[24px] md:rounded-[32px] bg-white/60 dark:bg-[#111]/60 border border-black/5 dark:border-white/5 backdrop-blur-md hover:bg-white/90 dark:hover:bg-[#181818] transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl flex flex-col gap-6 mb-6 mx-2 cursor-default">
    ...
    <div className="flex items-center gap-4">
      ...
      <div>
        <h4 className="font-bold text-black dark:text-white text-sm md:text-base leading-tight">
          {review.name}
        </h4>
        <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mt-0.5">
          {review.role}
        </p>
      </div>
    </div>
      ...
      <div className="flex gap-1">
        {[...Array(review.stars)].map((_, i) => (
          <Star
            key={i}
            size={12}
            className="fill-brand-red text-brand-red md:w-[14px] md:h-[14px]"
          />
        ))}
      </div>
    ...
    <div className="pt-6 border-t border-black/5 dark:border-white/5 flex items-center justify-between gap-2">
      <div className="flex items-center gap-2 opacity-60 grayscale group-hover:grayscale-0 transition-all duration-300">
        ...
        <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-black dark:text-white truncate max-w-[100px]">
          {review.company}
        </span>
      </div>
      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 group-hover:bg-emerald-500 group-hover:text-white transition-colors duration-300 shrink-0">
        <TrendingUp
          size={12}
          className="text-emerald-600 dark:text-emerald-400 group-hover:text-white"
        />
        <span className="text-[10px] md:text-xs font-bold text-emerald-700 dark:text-emerald-400 group-hover:text-white">
          {review.metrics}
        </span>
      </div>
    </div>
...
export const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section className="relative w-full py-24 md:py-32 overflow-hidden bg-[#F0F0F2] dark:bg-[#000000]">
      {/* Modale d'avis */}
      {isReviewModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 md:p-8 animate-enter-zoom">
          ...
          <div className="relative w-full max-w-2xl bg-white dark:bg-[#0A0A0A] rounded-[40px] border border-black/5 dark:border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.5)] overflow-hidden">
            <button
              onClick={() => setReviewModalOpen(false)}
              className="absolute top-6 right-6 w-10 h-10 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center hover:bg-brand-red hover:text-white transition-all z-50"
            >
              <X size={20} />
            </button>
            ...
            <div className="p-8 md:p-12">
              {isSuccess ? (
                <div className="py-20 text-center animate-fade-in-up">
                  <div className="w-20 h-20 rounded-full bg-emerald-500/10 flex items-center justify-center mx-auto mb-6 text-emerald-500 shadow-lg shadow-emerald-500/20">
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-8">
                  ...
                  <div className="relative group">
                    <MessageSquare
                      className="absolute left-4 top-5 text-gray-400 group-focus-within:text-brand-red transition-colors"
                      size={18}
                    />
                    <textarea
                  ...
                  <button
                    ...
                    className="w-full py-5 bg-black dark:bg-white text-white dark:text-black font-black uppercase text-[11px] tracking-[0.4em] rounded-2xl shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-4 group/btn disabled:opacity-50"
                  >
                    {isSubmitting ? (
