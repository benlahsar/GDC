...
export const HostingSolutionsGrid: React.FC = () => {
  const t = useTranslations("HostingSolutionsGrid");
  ...
  const SOLUTIONS = [
    {
      ...
      icon: Search,
      desc: t("solutions.strategy.desc"),
      points: t.raw("solutions.strategy.points")
    },
    {
      id: 2,
      title: t("solutions.security.title"),
      icon: Lock,
      desc: t("solutions.security.desc"),
      points: t.raw("solutions.security.points")
    },
    {
      id: 3,
      title: t("solutions.management.title"),
      icon: Server,
      desc: t("solutions.management.desc"),
      points: t.raw("solutions.management.points")
    },
    {
      id: 4,
      title: t("solutions.performance.title"),
      icon: TrendingUp,
      desc: t("solutions.performance.desc"),
      points: t.raw("solutions.performance.points")
    }
  ];
    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 max-w-[1600px] relative z-10">
        ...
        <div className={`text-center mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <h2
            className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter text-black dark:text-white mb-8 leading-tight max-w-5xl mx-auto"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {SOLUTIONS.map((item, index) => (
            <div
              key={item.id}
              className={`group relative flex flex-col p-8 md:p-10 rounded-[32px] bg-[#F5F5F7] dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 transition-all duration-500 ease-out overflow-hidden ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} ${!isMobile ? 'hover:border-brand-red/20 dark:hover:border-brand-red/20 hover:shadow-2xl hover:-translate-y-2' : 'shadow-md'}`}
            ...
            >
              <div className="relative z-10">
                ...
                <ul className="space-y-4 mt-auto">
                  {(item.points as string[]).map((point, idx) => (
